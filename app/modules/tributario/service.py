...
# ============================================================
# IMPORTAÇÃO DE PLANILHAS DE PRODUTOS DO TINY
# ============================================================

CABECALHOS_IMPORTACAO_TINY = {
    "id": {"id", "id tiny", "id produto", "id do produto"},
    "codigo": {"codigo", "codigo sku", "sku", "referencia", "codigo do produto"},
    "descricao": {"descricao", "nome", "produto", "nome do produto"},
    "ncm": {"classificacao fiscal", "ncm", "classificacao fiscal ncm"},
    "cest": {"cest", "codigo cest"},
    "fornecedor": {"fornecedor", "nome fornecedor", "fornecedor principal"},
    "gtin": {"gtin ean", "gtin", "ean", "codigo de barras", "gtin/ean"},
    "origem_mercadoria": {"origem", "origem da mercadoria", "origem mercadoria"},
    "unidade": {"unidade", "un", "unidade de medida"},
    "preco": {"preco", "preco venda", "preco de venda"},
    "custo": {"preco de custo", "preco custo", "custo", "custo medio"},
}


def _normalizar_cabecalho_planilha(valor: Any) -> str:
...
def _cabecalho_para_campo(valor: Any) -> Optional[str]:
    cabecalho = _normalizar_cabecalho_planilha(valor)
    if not cabecalho:
        return None

    for campo, aliases in CABECALHOS_IMPORTACAO_TINY.items():
        aliases_normalizados = {_normalizar_cabecalho_planilha(item) for item in aliases}
        if cabecalho in aliases_normalizados:
            return campo
    return None


def _valor_planilha_para_texto(valor: Any) -> str:
    if valor is None:
        return ""
...
def _normalizar_gtin(valor: Any) -> Optional[str]:
    texto = _valor_planilha_para_texto(valor)
    numeros = "".join(c for c in texto if c.isdigit())
    return numeros or None


def _normalizar_cest(valor: Any) -> Optional[str]:
    numeros = "".join(c for c in _valor_planilha_para_texto(valor) if c.isdigit())
    return numeros or None
...
def _ler_xlsx(conteudo: bytes) -> List[List[Any]]:
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Dependência openpyxl ausente. Adicione openpyxl ao requirements.txt.",
        ) from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Não foi possível ler o arquivo XLSX: {safe_str(exc)}",
        ) from exc


def _ler_xls(conteudo: bytes) -> List[List[Any]]:
    try:
        import xlrd
    except ImportError as exc:
        raise HTTPException(
            status_code=500,
            detail="Dependência xlrd ausente. Adicione xlrd==2.0.1 ao requirements.txt.",
        ) from exc

    try:
        workbook = xlrd.open_workbook(file_contents=conteudo, on_demand=True)
        planilha = workbook.sheet_by_index(0)
        return [planilha.row_values(indice) for indice in range(planilha.nrows)]
    except Exception as exc:
        raise HTTPException(
            status_code=400,
            detail=f"Não foi possível ler o arquivo XLS: {safe_str(exc)}",
        ) from exc
...
def _ler_csv(conteudo: bytes) -> List[List[Any]]:
    ...
    for encoding in ("utf-8-sig", "latin-1"):
        try:
            ...
            continue
    if texto is None:
        raise HTTPException(status_code=400, detail="Não foi possível decodificar o CSV.")
    ...
    try:
        ...
        dialect = csv.excel
        dialect.delimiter = ";"
    return [list(linha) for linha in csv.reader(io.StringIO(texto), dialect)]


def _ler_arquivo_produtos(nome_arquivo: str, conteudo: bytes) -> List[List[Any]]:
    nome = safe_str(nome_arquivo).lower()
    if not conteudo:
        raise HTTPException(status_code=400, detail=f"Arquivo vazio: {nome_arquivo}")
    ...
    raise HTTPException(
        status_code=400,
        detail=f"Formato não aceito em '{nome_arquivo}'. Use .xls, .xlsx ou .csv.",
    )


def _extrair_registros_planilha(
    nome_arquivo: str,
    conteudo: bytes,
) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    linhas = _ler_arquivo_produtos(nome_arquivo, conteudo)
    if not linhas:
        return [], {"arquivo": nome_arquivo, "linhas": 0, "cabecalho_encontrado": False}

    indice_cabecalho = None
    mapa_colunas: Dict[int, str] = {}

    # Procura o cabeçalho nas primeiras 20 linhas, pois alguns relatórios
    # possuem título ou linhas vazias antes da tabela.
    for indice, linha in enumerate(linhas[:20]):
        mapa_teste: Dict[int, str] = {}
        for coluna, valor in enumerate(linha):
            campo = _cabecalho_para_campo(valor)
            if campo and campo not in mapa_teste.values():
                mapa_teste[coluna] = campo
        if "codigo" in mapa_teste.values() and (
            "ncm" in mapa_teste.values() or "id" in mapa_teste.values()
        ):
    ...
    if indice_cabecalho is None:
        raise HTTPException(
            status_code=400,
            detail={
                "erro": "Cabeçalho da exportação do Tiny não encontrado.",
                "arquivo": nome_arquivo,
                "esperado": "Código/SKU e Classificação fiscal/NCM ou ID.",
            },
        )
    ...
    for linha in linhas[indice_cabecalho + 1:]:
        ...
        id_tiny = _valor_planilha_para_texto(registro.get("id"))
        codigo = _valor_planilha_para_texto(registro.get("codigo"))
        descricao = _valor_planilha_para_texto(registro.get("descricao"))

        if not id_tiny and not codigo and not descricao:
            ignoradas_vazias += 1
            continue
    ...
    return registros, {
        ...
        "linhas_vazias_ignoradas": ignoradas_vazias,
        "cabecalho_encontrado": True,
        "campos_identificados": sorted(set(mapa_colunas.values())),
    }


def _chave_produto_importado(registro: Dict[str, Any]) -> str:
    id_tiny = safe_str(registro.get("id"))
    codigo = safe_str(registro.get("codigo")).lower()
    if id_tiny:
        return f"id:{id_tiny}"
    if codigo:
...
def importar_produtos_tiny(
    ...
    if not arquivos:
        raise HTTPException(status_code=400, detail="Envie ao menos uma planilha.")
    if len(arquivos) > 20:
        raise HTTPException(status_code=400, detail="Envie no máximo 20 arquivos por importação.")
    ...
    for nome, conteudo in arquivos:
        registros, detalhe = _extrair_registros_planilha(nome, conteudo)
        registros_brutos.extend(registros)
        detalhes_arquivos.append(detalhe)

    # O último registro repetido prevalece, permitindo importar exportações
    # sequenciais sem gerar atualizações duplicadas.
    unicos: Dict[str, Dict[str, Any]] = {}
    sem_identificador = 0
    duplicados = 0
    for registro in registros_brutos:
        chave = _chave_produto_importado(registro)
        if not chave:
            sem_identificador += 1
            continue
        if chave in unicos:
    ...
    produtos_banco = supabase_get_todos(
        [
            (
                "select",
                "id,filial,tiny_produto_id,codigo,sku,descricao,ncm,cest,"
                "origem_mercadoria,unidade,preco,custo,fornecedor,gtin,ativo,"
                "status_ia,percentual_confianca,justificativa_ia,data_analise,"
                "lote,ultima_sincronizacao",
            ),
    ...
    por_tiny_id = {
        safe_str(item.get("tiny_produto_id")): item
        for item in produtos_banco
        if safe_str(item.get("tiny_produto_id"))
    }
    por_codigo = {
        safe_str(item.get("codigo")).lower(): item
        for item in produtos_banco
        if safe_str(item.get("codigo"))
    }
    por_sku = {
        safe_str(item.get("sku")).lower(): item
        for item in produtos_banco
        if safe_str(item.get("sku"))
    }
    ...
    for registro in unicos.values():
        id_tiny = safe_str(registro.get("id"))
        codigo = safe_str(registro.get("codigo"))
        codigo_chave = codigo.lower()

        existente = None
        encontrado_por = None
        if id_tiny and id_tiny in por_tiny_id:
            existente = por_tiny_id[id_tiny]
            encontrado_por = "tiny_produto_id"
        elif codigo_chave and codigo_chave in por_codigo:
            existente = por_codigo[codigo_chave]
            encontrado_por = "codigo"
        elif codigo_chave and codigo_chave in por_sku:
            existente = por_sku[codigo_chave]
            encontrado_por = "sku"

        if not existente:
            nao_encontrados.append({
                "tiny_produto_id": id_tiny or None,
                "codigo": codigo or None,
                "descricao": safe_str(registro.get("descricao")) or None,
            })
            continue
        ...
        alteracoes: Dict[str, Any] = {
            "filial": filial_normalizada,
            "tiny_produto_id": safe_str(existente.get("tiny_produto_id")),
        }

        ncm = ncm_limpo(registro.get("ncm"))
        if not ncm:
            sem_ncm_planilha += 1
        elif len(ncm) != 8:
            ncm_invalidos += 1
        elif ncm != ncm_limpo(existente.get("ncm")):
            alteracoes["ncm"] = ncm
            alteracoes["status_ia"] = "PENDENTE"
            alteracoes["percentual_confianca"] = None
            alteracoes["justificativa_ia"] = None
            alteracoes["data_analise"] = None

        campos_texto = {
            "cest": _normalizar_cest(registro.get("cest")),
            "fornecedor": safe_str(registro.get("fornecedor")) or None,
            "gtin": _normalizar_gtin(registro.get("gtin")),
            "origem_mercadoria": safe_str(registro.get("origem_mercadoria")) or None,
            "unidade": safe_str(registro.get("unidade")) or None,
        }
        if atualizar_descricao:
            campos_texto["descricao"] = safe_str(registro.get("descricao")) or None

        for campo, valor in campos_texto.items():
            if valor is not None and safe_str(existente.get(campo)) != safe_str(valor):
                alteracoes[campo] = valor

        if importar_precos:
            preco_texto = safe_str(registro.get("preco"))
            custo_texto = safe_str(registro.get("custo"))
            if preco_texto:
                preco = dinheiro_para_float(registro.get("preco"))
                if float(existente.get("preco") or 0) != preco:
                    alteracoes["preco"] = preco
            if custo_texto:
                custo = dinheiro_para_float(registro.get("custo"))
                if float(existente.get("custo") or 0) != custo:
                    alteracoes["custo"] = custo

        # Apenas chaves de conflito significam que nada mudou.
        if set(alteracoes) == {"filial", "tiny_produto_id"}:
            sem_alteracao += 1
            continue

        alteracoes["ultima_sincronizacao"] = agora
        alteracoes["updated_at"] = agora
        alteracoes["_encontrado_por"] = encontrado_por
        payloads.append(alteracoes)

    for inicio_lote in range(0, len(payloads), 100):
        lote = payloads[inicio_lote: inicio_lote + 100]
        lote_banco = []
        for item in lote:
            copia = dict(item)
            copia.pop("_encontrado_por", None)
            lote_banco.append(copia)
        salvos = supabase_upsert(lote_banco)
        atualizados += len(salvos) if salvos else len(lote_banco)
    ...
    return {
        ...
        "linhas_lidas": len(registros_brutos),
        "produtos_unicos": len(unicos),
        "duplicados_removidos": duplicados,
        "linhas_sem_identificador": sem_identificador,
        "produtos_encontrados": encontrados,
        "produtos_atualizados": atualizados,
        "produtos_sem_alteracao": sem_alteracao,
        "produtos_nao_encontrados": len(nao_encontrados),
        "nao_encontrados_amostra": nao_encontrados[:30],
        "produtos_sem_ncm_na_planilha": sem_ncm_planilha,
        "ncm_invalidos_na_planilha": ncm_invalidos,
        "importou_precos": importar_precos,
        "atualizou_descricao": atualizar_descricao,
        "campos_importados": [
            "ncm", "cest", "fornecedor", "gtin", "origem_mercadoria", "unidade",
            *( ["preco", "custo"] if importar_precos else [] ),
            *( ["descricao"] if atualizar_descricao else [] ),
        ],
        "duracao_segundos": duracao,
        "observacao": (
            "Campos vazios na planilha não apagam dados existentes. "
            "O produto é localizado primeiro pelo ID Tiny e depois por código/SKU."
        ),
    }
